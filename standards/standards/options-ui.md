...
- **Test mode belongs here too, when the addon has one.** An addon whose preview (preview-mode) has a switch of its own — a test mode the player turns on and that stays on until turned off, reached by a `/<slash> test` or `/<slash> preview` verb — **MUST** carry a **`Test mode`** checkbox as the row below *Lock frame* / *Debug console*, starting its own line, with the right-hand cell empty. It is **session-only**, like the console row: ticked exactly while the test mode is on, cleared when the test mode ends by itself (combat, a roster change, the addon switched off), and left unticked when a start is refused. It is **composed, never hand-written**: the `MasterControls` composer emits it when the host passes `testModePath` (LibKa0s v1.37.0, compose minor 6), and the host binds the path's get/set to its test mode exactly as it binds the console's. **Omit it when the addon has no such mode.** A preview reached only by unlocking already has its switch in *Lock frame*; a one-shot test action — a sample line printed, a flow run once, a fake value held for a few seconds — is not a mode, stays a verb, and may take the composer's `leadButton` beside the resets. A test mode reachable only from chat, or drawn as a button that cannot show whether it is on, is **anti-pattern #80**.
