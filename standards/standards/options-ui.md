...
- **MUST** vendor **`LibKa0s-Options-1.0`** — the whole `LibKa0s/` folder, never a hand-picked subset — under `libs/LibKa0s/`, listed in the TOC's `# Libraries` section after Ace3 (toc-file-§4, library-stack-§7). The major is multi-file and depends on `LibKa0s-Core-1.0`. **This section keeps no file list**: library-stack-§7's module table is the single inventory of which files make the major up, and the count written here drifted three times because it was a second copy of that table. Copy the shell without its attach files and you get a panel shell whose widget makers, page chrome, composers and scrollbar patch are simply not there — and it registers and answers `:New` anyway, so the absence surfaces at call time (**anti-pattern #48**).
...
- **MUST** degrade rather than error when the library is absent — see the degradation rule at the end of this section. It departs from the member-answering stub a Ka0s setup file uses by default: it is **load-completing**, one of the named stub shapes this section sets beside that default. The others are not Options': the **runtime-completing** stub a major's own API document prescribes, `LibKa0s-Compat-1.0`'s **reader** and **guard** arms and `LibKa0s-Bus-1.0`'s **untracked-target** stub, all closing this section.
...
**Degradation — LOAD-COMPLETING, not member-answering (MUST).** A Ka0s setup file falls back, by default, to a stub whose members each print an honest *"not installed"* line. This section names the shapes that depart from that default for the majors it rules on: this one, and, closing the section, the runtime-completing stub, Compat's reader and guard arms and Bus's untracked-target stub. The list is **not exhaustive**. A stub rule another section sets (slash-commands-§1, debug-logging-§7, performance-§1) stands as that section writes it, and a stub for a major no section rules on yet stands as it is: the collection's working `LibKa0s-Lifecycle-1.0` stubs, which complete the latch, are unaffected by anything here. This one **MUST NOT** take the default, and the reason is not importance but **when** the missing code is reached: page files call `Helpers.LSMValues("statusbar")` *inside schema-row literals, at file load*. With that member nil the page file raises, so its `RegisterSchemaRows` never runs, so a third of the schema is missing — and `list`, `get`, `set`, `reset` and the profile defaults all break with it, silently. The addon would not degrade; it would half-load and say nothing.
...
**A RUNTIME-COMPLETING stub, where the major's own API document prescribes one (MAY).** The member-answering default assumes the missing major is reached from the panel, the CLI or a diagnostic, where one honest line is the right answer. A major the addon's **feature runtime** and its **host writers** both reach is not in that position. `LibKa0s-Schema-1.0`'s seam is read on a repaint pass and written by the host verbs slash-commands-§1 keeps working, by the *Reset All* this section asks the Options stub to keep real, and by a combat re-lock; a stub whose `Get` printed *"not installed"* would leave the addon unable to read its own settings, and a load-completing stub does not reach a per-frame read.

The class is **tied to the library, not to a host's reading of its own runtime.** It is the class of stub that completes a major's **stored reads and writes**, and it is open to a major whose own API document's degradation section prescribes such a stub. Today that is `LibKa0s-Schema-1.0` alone (`LibKa0s docs/api/Schema/version-1-docs.md`, *The degradation stub*). A stub for any other major **MAY** take the shape that major's own API document prescribes, and it is held to that document, not to this class. Compat's two arms are named below. `LibKa0s-Bus-1.0`'s is the host stub in its API document's *Worked example* (`docs/api/Bus/version-1-docs.md`), and it is a named shape here too: **the untracked-target stub**. Its `New` answers a record whose `NewTarget` still hands each receiver a private AceEvent target, untracked, and answers `nil` only when `AceEvent-3.0` itself is absent; its `StandDown` and `StandUp` answer `0` and `0, {}`; its `Catalog` hands back the host's own table. It records nothing and prints nothing, and it completes no stored read or write, so it is not runtime-completing. The `nil` in that document's *Answers with a dependency absent* table is the **live** library's answer with AceEvent missing, not a stub's; a Bus stub whose `NewTarget` answered `nil` while AceEvent is present would leave every receiver unregistered on the degraded load. A host cannot admit a stub for another major to the runtime-completing class by arguing that its runtime reaches the major. Whatever governs a stub already (its own section's rule, the shape its major's API document prescribes, or, for a major neither rules on, the shape it has) is unaffected by this paragraph. For a major in the class, the host's library-absent stub **MAY** be runtime-completing:

- **Reads and writes complete.** A read answers the stored value; a write validates, stores, reacts and announces, as the live member does.
- **Nothing that only feeds the debug console completes.** The stub never logs a per-write line and never keeps a count only the console reads, such as the bulk bracket's tally or the profile-reset count. Those lines exist to feed the console window, which is what a degraded load loses (debug-logging-§7), and debug-logging-§10 carries the matching exemption for this stub. The one *library missing* line library-stack-§7 asks the setup file for is unchanged.
- **It is a deliberate, documented duplication**, in library-stack-§7's sense. The host copy carries a comment saying so and citing the major's API document's degradation-stub section, and its member set is held to the live surface like any other stub's (anti-pattern #56).

**`LibKa0s-Compat-1.0`'s two arms, as its API document prescribes them.** The Compat API document's *Degradation* section gives a host's stub two arms, and both are named shapes here. **The reader arm**: a reader (`GetSpellInfo`, `GetSpellName`, `GetSpecialization` and the rest) answers the absent value that section's absent table gives (`nil`, or `0, 0, false, 1, false`) and prints nothing. It carries no separate *library missing* line, because a Compat-only absence arises only from a partial copy, and a whole-payload absence is already announced by the host's setup file. It copies nothing. **The guard arm**: `IsSecret` / `CanAccess` / `IsSafeKey` re-implement the one-rung body, because a guard answering *no secrets* on a 12.x client sends a secret into a comparison on exactly the degraded path. The host copy carries a comment naming that section.

**Neither a runtime-completing stub nor Compat's guard arm is the hand-rolling anti-pattern #47**, and neither Compat's reader arm nor Bus's untracked-target stub copies anything of the library at all. Both run only while the major is absent; #47 is a host copy that runs **instead of** the library. The bound is the no-copy **MUST NOT** above, applied to every shape this section names and to copies of the library's own code only: none of them carries a copy of a widget maker, the flow engine, the header, a layout constant, a composed block, a debug console or its formatters, the library's slash dispatcher, help renderer or value parser, or a test harness. A Slash stub's routing of the host verbs and its plain help rows are slash-commands-§1's to rule, and are not a copy in this sense. These shapes are permissions and require nothing: a host that keeps the member-answering default for a major in the class is not out of compliance by this section, and what a major's own API document asks of an adopter is that document's to ask.
...
- **MUST NOT** close, hide or commit Blizzard's settings window from addon code in combat — no `SettingsPanel:Close()`, no `HideUIPanel(SettingsPanel)`, no `ToggleGameMenu()`, no `Settings.OpenToCategory` — and no read or write of `SettingsPanel`'s own fields while `InCombatLockdown()`. Gating the open does **not** keep a page from being shown in combat: Blizzard's AddOns sidebar reaches a registered page's `OnShow` without passing through the addon's open function. A close run from that `OnShow` executes Blizzard's close-and-commit path from addon code, tainted — `SaveBindings()` is blocked (`ADDON_ACTION_BLOCKED`), then `ToggleGameMenu` re-enters the half-shown panel's close until the client reports `C stack overflow` and the window is left broken (**anti-pattern #88**).
- **A page shown in combat is LOCKED.** A cover over the **whole** page — the header band and the tab strip included — states it in one centered gray line, **"Settings are locked during combat."**, and takes the mouse so nothing beneath it can be clicked, dragged or typed into. Every write through the options surface is **refused** until `PLAYER_REGEN_ENABLED`: widget writes, the page's **Defaults** button, *Reset all settings* (options-ui-§12), structural re-renders, and tab switches (options-ui-§13). The same lock falls over a page that is already open when combat starts. When combat ends the cover lifts and the page on screen renders from **current** state, so a value changed meanwhile by a slash verb or a profile switch shows up — and nothing is re-opened: the no-defer-and-replay rule above binds the lock exactly as it binds the open.
- **The lock is owned by the options library, not by each host.** `LibKa0s-Options-1.0` carries the cover, the refusal at its write seam and the render on `PLAYER_REGEN_ENABLED` (from LibKa0s **v1.46.0**); a host **MUST NOT** hand-roll a second **page-level** lock beside it — its own cover, its own refusal to render the panel or a page, its own tab guard, its own close. The prohibition stops at the page: a setter-level `InCombatLockdown()` gate on a setter that creates, destroys or rebuilds frames is not a second lock, and it stays under the **SHOULD** below. The lock covers the settings window only: a consumer's slash `set` / reset verbs are not the options surface and keep the addon's own combat rules.
- **SHOULD** apply the same `InCombatLockdown()` gate to any settings setter that creates, destroys or rebuilds frames — **including** a setter reachable only from the settings page. The library's lock refuses what passes through its write seam, but it cannot refuse every host-owned callback: a drag already in flight in a reorder list (options-ui-§18) when combat starts commits through the host's own code. Such a gate is not the second page-level lock the bullet above forbids. Example: Ka0s Multi Meters' `settings/Columns.lua` `commit()` refuses with *Columns cannot be changed during combat.* rather than rebuild the meter window's cells while they hold secret values (events-frames-taint-§8).

**The same call, outside combat.** The bullets above govern *when* `Settings.OpenToCategory` may be made; these two govern *how*, and they sit here because a reader who has found one has found the place the other should be. Four addons in the collection each wrote both up in their own quirks file before either reached the standard.

- **`Settings.OpenToCategory` takes the numeric category ID (MUST).** `Settings.RegisterCanvasLayoutCategory` and `Settings.RegisterCanvasLayoutSubcategory` each return a category object whose `:GetID()` is the auto-assigned integer the call accepts; `AceConfigDialog:AddToBlizOptions` returns `(frame, categoryID)` on a modern client, and it is the **second** value. Passing the category object — or the frame — raises a range error. Capture the ID with `:GetID()` at registration time and open against that. **MUST NOT** overwrite `category.ID` with a string: that form is not accepted either, and unlike the other two it fails **silently** — the lookup misses and the open becomes a no-op, with no error to find it by.
- **Opening a *parent* leaves its subcategory tree collapsed (MUST).** Blizzard's category list auto-expands a parent's tree only when a **child** is selected, so an open against the landing page (options-ui-§5) hides exactly the sibling tabs the player is navigating to. Force it with `SettingsPanel:GetCategoryList():GetCategoryEntry(category):SetExpanded(true)`. The walk reaches the **list entry** rather than the category because `SettingsCategoryMixin` exposes no `SetExpanded` — that method lives on the visual list-entry element — which is why a shorter walk against the category object cannot work. Two constraints make it work at all: the expand call goes **after** `Settings.OpenToCategory`, so `SettingsPanel` is realized and the entry element exists; and it is re-run on **every** open, so a collapse the player made mid-session does not carry into the next `/<slash> config`. Every step is private Blizzard API, so the whole walk sits under `pcall` and degrades to *parent opened, tree collapsed* — the behavior without the fix — rather than raising.
- **Both are the library's, not the host's (MUST NOT hand-roll the open).** `LibKa0s-Options-1.0` captures the parent's ID at registration and runs the expand walk after the open, once, for every addon; a host **MUST NOT** wire its own panel-open beside it. A second open is a second place for the ID capture, the ordering constraint and the `pcall` to drift, and four separate write-ups of the same two facts are what that drift looked like before the extraction. **The carve-out is the page jump**: a combat-gated open against an ID the host recorded for one of its **own** subcategories, to land the player on a named page, meets neither problem — the ID is a subcategory's and the tree expands itself — and is not the second open this forbids.
...
- **Logo asset:** ship a **`.tga`** (or `.blp`) under **`media/logos/`** — WoW **cannot** load `.jpg`/`.png` textures at runtime. Reference it by absolute path `Interface\AddOns\<Folder>\media\logos\<name>.tga`, display at **300×300**; source art SHOULD be power-of-two (e.g. 512×512). Keep the original `.jpg`/`.png` beside it for editing. **This is the landing page's own asset and is NOT the icon file:** the 128×128 `<addon>.logo.128.tga` that the TOC's `## IconTexture`, the minimap button and the broker object share is a second, smaller file in the same folder (layout-§4, launcher-§4). Two files, two jobs — do not point one at the other.
...
- **Switching tabs is a structural re-render**, and in combat it is refused with the rest of the page. options-ui-§2's combat lock covers the **tab strip** along with everything else on a page shown in combat, so a tab click does nothing until `PLAYER_REGEN_ENABLED`. The **library** owns that refusal, as it owns the rest of the lock; a host still **MUST NOT** add a tab guard of its own — a second guard is a second place for the lock to disagree with itself. Out of combat, what a tab switch inherits is the re-render path itself — the same one a change of subject takes. *(Until v2.60.0 this bullet said a tab click works in combat and forbade any guard that refuses one. The collection owner chose to lock the whole page instead, after the incident anti-pattern #88 records.)*
...
- **`Enable <AddonName>` is the addon-wide switch, and the slash surface writes the same path.** `/<slash> enable` and `/<slash> disable` are reserved aliases onto this row's stored path, through this panel's own write seam — they are not a second switch and hold no state of their own (slash-commands-§2). The checkbox therefore reflects a `/<slash> disable` immediately, because there was only ever one value. **What the addon does once that value is `false` is slash-commands-§7**: the disabled state is **total** — frames hidden, every registration unregistered, every timer canceled, no write from any game event — while the slash surface keeps answering in full (slash-commands-§2), `config` and the bare `/<slash>` included. This row, reached through those verbs or through Blizzard's own Options → AddOns tree, is how a player turns the addon back on by hand, so the settings-category registration stays up while the addon is off.
...
- **Minimap button belongs here, in every addon, and it takes the FIRST column of its line.** Every Ka0s addon ships a launcher (launcher), so the row is unconditional: a **`Minimap button`** checkbox that shows and hides the minimap button, stored as LibDBIcon's own `minimap.hide` and written through the addon's single write seam like every other row — the storage, the inversion and the `Show` / `Hide` call are launcher-§3's and are not restated here. **The column assignment is deliberate, and it is the asymmetry that decides it:** *every* addon has a minimap button and only *some* have a test mode, so the always-present row goes in column 1 and *Test mode* pairs beside it in column 2. An addon without a test mode then draws a tidy single row rather than a hole in the first column with a lone control to its right. Do not "fix" the order to put the optional row first; it looks arbitrary and is not.
- **Test mode belongs here, in every addon with a positionable display.** An addon whose display can be positioned (preview-mode applies to it) **MUST** ship a **test mode** — placeholder content on its display, turned on and left on until turned off — and its switch **MUST** be a **`Test mode`** checkbox in the **second** column of the row below *Lock frame* / *Debug console*, beside *Minimap button* (whose first-column position the bullet above explains). The mode is **session-only**: never persisted, off after `/reload`, and ended by *Reset all settings* (the row declares `default = false`). It **shows** the display without requiring an unlock — it MAY unlock as a side effect, and a verb such as `/<slash> test` MAY drive the same switch — and **it ends when combat starts** (`PLAYER_REGEN_DISABLED`, while secure writes are still allowed), so no placeholder covers real data in a fight — and for the same reason **a start during combat is refused**, with one line saying why. The checkbox follows every start and stop, and a refused start leaves it unticked. It is **composed, never hand-written**: the `MasterControls` composer emits it from `testModePath` (LibKa0s v1.37.0), and the host binds its get/set the way it binds the console row's. A one-shot test action (a sample line printed, a flow run once, a value held for a few seconds) MAY stay beside it as a verb or the composer's `leadButton`, but does not replace it. **Two kinds of addon omit the row.** A frameless addon, which draws no positionable display. And an addon whose **unlocked view already is its test mode** — unticking *Lock frame* shows the display with its placeholder content — so *Lock frame* is the switch and a second one would only duplicate it; such an addon ships no `/<slash> test` verb either, and `/<slash> lock` / `unlock` are its verbs. A positionable display with no Test mode checkbox, or with its test mode drawn as a button or reachable only from chat, is **anti-pattern #80**.
