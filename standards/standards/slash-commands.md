...
- **`list`** **MUST** print a header, then one **`[page]` group header** per schema page (in a stable, declared page order), then one indented **`path = value`** row per setting under that page (color codes shown stripped for legibility — see the color scheme below):
...
- **Color scheme (MUST).** Every Ka0s addon prints schema output in **one shared color scheme** so the surface reads identically across the collection (the same house-style intent as the mandated cyan chat tag, slash-commands-§4). No line **MUST** carry a **trailing colon**.
  ...
  The ` = ` separator stays default (uncolored). The `NS.PREFIX` tag keeps its mandated cyan (slash-commands-§4). These four colors are **mandatory, not merely examples** — **MUST NOT** substitute other colors or add a trailing colon.
- **`get <path>`** and **`set <path> <value>`** **MUST** print the **single-line** `path = value` form (no header, no indent) — using the **same gold-key / white-value coloring** as the `list` value rows, produced by the same shared `key = value` formatter (below). A `set` **MUST** read back the *stored* value after writing, so the echo reflects any clamping/coercion.
...
- A **single shared value formatter** (e.g. `NS.FormatSchemaValue(row, v)`) **MUST** produce these value strings for both `list` and `get`/`set`, so the two paths can never diverge. Likewise the colored **`key = value` line** (gold key + white value) **MUST** come from **one shared helper** (e.g. `FormatKV(path, valueStr)`) reused by the `list` rows and the `get`/`set` echo, so the coloring can never drift between them.
