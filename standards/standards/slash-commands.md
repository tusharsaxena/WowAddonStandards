...
- **`runDebug` tests `diagnostics` first** (debug-logging-§5, debug-logging-§14), so `/<slash> debug diagnostics` and the `diagnostics` row run the same report through the same ungated sink. `NS.DebugLog:DebugVerb(rest)` routes `diagnostics`, `on` and `off` and returns `false` for any other word, which leaves the window toggle as the host's fallback:

```lua
runDebug = function(rest)                          -- the forward-declared host verb
  if NS.DebugLog:DebugVerb(rest) then return end   -- `diagnostics` first, then `on` / `off`
  NS.DebugLog:Toggle()                             -- bare `debug`, and any word it does not know
end
```
...
`diagnostics` and `perf` run, and the whole schema CLI — `get`, `set`, `list`, `reset`,
`resetall` — reads and repairs settings, which is precisely when a player most needs it. §2's
**SHOULD** that a *feature* verb answers one tagged line naming `/<slash> enable` stands unchanged:
it was written before this section existed, it survived the reversal, and it is the only refusal in
the disabled state.
...
The button stays on the minimap and the broker row stays in the display (*What MUST survive*), because their visibility is a separate preference. What the two clicks do:

- **Left-click still opens the settings panel**, unchanged, on every addon, in either state (launcher-§2). The panel is **setup**, not a feature (*What MUST survive*), so the left button opens it for the same reason `config` and the bare `/<slash>` still do (slash-commands-§2); refusing it would make the mouse the one surface on which the off switch is harder to reach than from chat.
- **Right-click still opens the options menu, and its *Enabled* entry stays live.** *Enabled* is the off switch, and like `/<slash> enable` it has to work in the off state. **Every other entry — *Locked*, *Test mode*, *Show window* — is grayed out** with the note *enable the addon first*, because each drives a feature (launcher-§2). A grayed entry is shown rather than hidden, so the player sees what the addon offers and the step before it, and it calls **no** handler.
- **A launcher click MUST NOT write SavedVariables while the addon is disabled**, except the one write the player asks for by ticking *Enabled*, which is the enable path itself. The audit found an addon whose minimap button stays clickable with **no disabled gate at all**, so clicking it writes the stored tree of an addon the player has switched off. That is a bug under every version of this rule, and it is named here because nothing previously said so: the click is a game event in every sense that matters, and *What MUST stand down* reaches it. The library's graying is what enforces it (Launcher minor 4, `LibKa0s v1.58.0`).
- **Until v2.67.0 this subsection refused the left-click instead.** The left button then followed launcher-§2's three rungs, and on rungs (a) and (b) — a primary window, a preview switch — it printed *The refusal line* and did nothing else, while rung (c)'s left-click, already the settings panel, was left alone. v2.67.0 retired the rungs, so left-click is the settings panel everywhere and there is nothing on it to refuse; the features the rungs drove are now grayed menu entries.

#### The refusal line — one shape, collection-wide

The one refusal a disabled addon prints — §2's feature-verb SHOULD — is **exactly one line**, through the addon's own tagged printer (slash-commands-§4), in this shape:
...
7. **The slash surface.** Dispatch **every** entry in `NS.COMMANDS` through the real dispatcher while disabled. Assert that the reserved verbs and the schema CLI — `help`, `config`, `version`, `enable`, `disable`, `debug`, `diagnostics`, `perf`, `get`, `set`, `list`, `reset`, `resetall` — and the bare `/<slash>` all produce their **normal** output, because slash-commands-§2 **MUST**s that they keep answering. Assert that any verb the addon implements §2's feature-verb **SHOULD** for produces **exactly one line** matching *The refusal line* and reaches **no write seam**. An addon that declines that SHOULD asserts its feature verbs act normally instead; either is conformant, and the suite pins whichever this addon chose so the choice cannot drift silently. **This step is not the stand-down** — steps 1–6 are. A green step 7 says nothing about whether the addon is inert.

8. **The launcher.** Drive the LDB object's `OnClick` with `"LeftButton"` and assert the panel opener was called, with **zero** SavedVariables writes and zero frame shows. Drive it with `"RightButton"` through the kit's headless menu mock and assert the options menu opened with *Enabled* clickable and every other entry the addon has grayed, and that opening it wrote nothing and showed no frame (launcher-§2). Until v2.67.0 this step asserted a refused left-click and a right-click on the panel.
...
- **Where unlocking is the preview, the verbs drive that same switch** and never a copy of it — the preview follows from the stored lock exactly as it does when the checkbox is clicked (preview-mode). The launcher menu's *Locked* entry drives the same handler the verbs do (launcher-§2). An `unlock` verb that shows a preview by some other route is a second preview state to keep in step.
