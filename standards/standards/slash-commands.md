...
- **The dispatcher and the settings registration survive the disabled state, so the pair is never one-way.** `/<slash>` keeps being answered by something while the addon is **disabled**: the addon **MUST NOT** unregister its chat command, tear down its `COMMANDS` table or drop its dispatcher. An addon that does any of those has built a switch that only goes one way — the player turns it off and the verb that turns it back on no longer exists. The chat command, the dispatcher and the settings-category registration are **setup**, not features: they come up on load in either state and stay up. **What is answered through that surviving dispatcher is no longer this bullet's to say** — it is slash-commands-§7, which reverses what this section used to rule and states the reversal in as many words.
...
- **Every event, message and bucket registration the addon owns is actually UNREGISTERED.** Every `RegisterEvent`, `RegisterUnitEvent`, `RegisterMessage`, `RegisterBucketEvent` and raw `frame:RegisterEvent` the addon made, on every frame and every AceEvent target it owns, including the per-unit frames — gone, not gated.
...
- **The settings-category registration and the panel body.** The addon stays listed in Blizzard's Options → AddOns tree while disabled, and its *Enable `<AddonName>`* checkbox is live there. This is not a nicety: the slash-surface subsection above refuses the `config` verb, and this registration is what replaces it.
...
#### The slash surface while disabled — `enable` and `help`, and nothing else

**This reverses slash-commands-§2 as it stood from v2.53.0 through v2.55.0, and the reversal is stated rather than left for a reader to trip over.** The old rule **MUST**ed that the dispatcher, `config`, `version`, `debug`, `perf` and the entire schema CLI — `get`, `set`, `list`, `reset`, `resetall` — keep answering while disabled, and made refusal of the remaining feature verbs a **SHOULD**. Its reasoning was not stupid and is worth recording, because it was a real trade: a player must be able to **read and repair settings** and **reach the panel** precisely when the addon is off, which is when they are most likely to need to; and `debug` and `perf` are diagnostics rather than features, reached for exactly when the addon is misbehaving. That trade has been **overruled by the owner, explicitly.** The judgment is that a disabled addon which still answers twelve verbs is not disabled — it is a running addon with its picture turned off — and that the clarity of *off means off* is worth more than the convenience of repairing settings without first typing one word. The rule below is now what the standard says; the paragraphs it replaces have been rewritten in slash-commands-§2 rather than left standing beside it, because a spec that MUSTs both is worse than either rule alone.

- **Exactly two verbs answer normally while the addon is disabled: `enable` and `help`.** Nothing else. There is no third case and no per-addon exception.
- **`enable`** writes the stored path and stands the addon up (slash-commands-§2), confirming in slash-commands-§5's `set` shape.
- **`help`** prints the full help index, generated from `COMMANDS` exactly as it is when enabled — the player has to be able to *see* the verb list to find `enable` in it — and **MUST** print the refusal line (*The refusal line*, below) immediately after the header, unindented, so the index does not read as twelve working commands.
- **Every other verb — feature verb, `config`, `version`, `debug`, `perf`, `get`, `set`, `list`, `reset`, `resetall`, and any host verb the addon defines — answers with the single refusal line (*The refusal line*, below) and does nothing else.** No partial work, no side effect, no second line, no state read.
- **Bare `/<slash>` is refused.** With the addon enabled it opens the panel on its landing page (slash-commands-§3); while disabled it is `config`'s act by another spelling, and `config` is refused, so the bare form refuses identically. Ruling it the other way would leave the bare command as an undocumented back door into a verb the owner's list excludes.
- **An unknown verb while disabled prints the same refusal line**, not `unknown command '<verb>'` followed by the help index. While disabled there is exactly one thing the player can usefully be told, and distinguishing *I do not know that verb* from *I am switched off* tells them nothing they can act on. slash-commands-§3's unknown-verb rule is the enabled-state rule.
- **`/<slash> disable` while ALREADY disabled is a no-op write, not a refusal, and it echoes.** It is not a feature verb, so this subsection's refusal is not turned on it; and it is an alias onto a schema write, so writing `false` over `false` is idempotent and its honest answer is slash-commands-§5's `path = value` echo of the stored value. Refusing it would answer `/<slash> disable` with a line telling the player to type `/<slash> enable`, which reads as the addon having misunderstood the request. Symmetrically, `/<slash> enable` while already enabled echoes rather than erroring. Neither may re-run the stand-down or stand-up work — the latch below already makes the transition idempotent, and a second stand-down that unregisters an already-empty registration set is how a double-release bug gets written.

**What the refusals cost, said plainly rather than argued away.** Three of them are real losses and the standard does not pretend otherwise.

- **`config` was protected so the player could reach the panel.** It is no longer protected, and the panel route it provided is replaced by two that are not the addon's dispatcher: **Escape → Options → AddOns → the addon's category**, which is Blizzard's own settings tree and which *What MUST survive* keeps the addon registered in; and **right-clicking the addon's minimap button or broker row**, which launcher-§2 already makes an unconditional route to the panel and which *The launcher while disabled* keeps working. The cost is a click and a loss of discoverability from chat. The repair itself is not lost: the *Enable `<AddonName>`* checkbox is the first row of the first tab of the page those routes open.
- **The schema CLI was protected so the player could read and repair settings from chat.** That is the largest thing given up here, and it was the old rule's entire reason. What replaces it is the panel, by the two routes above — and `/<slash> enable`, which is one word and puts the whole CLI back.
- **`debug` and `perf` were protected because the usual reason to reach for either is that the addon is misbehaving.** Refusing them **costs diagnosis of a disabled addon**, and that is a genuine loss on the day it bites. It is also a loss the rest of this section shrinks almost to nothing, and that is the honest defense rather than a dismissal: under the **old** draw-gate shape a disabled addon was still running, so `debug` and `perf` had real subject matter and real value. Under **this** section a conformant disabled addon registers nothing, runs nothing and writes nothing — there is no log to take and no cost to measure, and a `perf` capture against it would compare an inert arm with an inert arm. The workflow is one step longer: `/<slash> enable`, reproduce, `/<slash> debug`. A player diagnosing an addon has to turn it on first, which is what they were going to do anyway.
...
- **Right-click still opens the settings panel**, unchanged, on every addon, in either state. This is deliberately **not** inconsistent with refusing `config`: the owner's ruling is about the **slash surface** — *"the only valid slash command at that point"* — and a mouse click on a minimap button is not a slash command. It is also the route the slash-surface subsection nominates as `config`'s replacement, so refusing it would leave the panel reachable only through Blizzard's own tree.

#### The refusal line — one shape, collection-wide

Every refusal in the two subsections above prints **exactly one line**, through the addon's own tagged printer (slash-commands-§4), in this shape:
...
- **`perf` is refused while disabled** (*The slash surface while disabled*), so a capture cannot be *started* on a disabled addon. That is the right answer for its own reason: the harness's independent variable is *"does our code run?"*, and against a conformant disabled addon both arms are inert, so the capture would measure nothing and report it as a null result.
...
7. **The slash surface.** Dispatch **every** entry in `NS.COMMANDS` through the real dispatcher. Assert that **exactly `enable` and `help`** produce their normal output, and that **every other verb** produces **exactly one line** matching *The refusal line*'s shape and containing `/<slash> enable`. Dispatch a bare command and an unknown verb and assert the same single line. Dispatch `disable` and assert it echoes `<enablePath> = false` rather than refusing.
...
Every addon in the collection is **non-compliant with this section today**, all eleven of them, and that is the expected starting state rather than a sweep of surprises. The library seam lands **first**: this section's latch is `LibKa0s-Lifecycle-1.0` (library-stack's module table), and until a LibKa0s tag carries it an addon's adoption is **blocked rather than overdue**, exactly as launcher-§5's composer seam blocked the launcher. **The tag number is written into this subsection and into library-stack in the same change that ships it** — no version is named here in advance, because a number invented ahead of the release is a citation that does not resolve.
...
- **They are feature verbs, so the slash-surface subsection above refuses them while the addon is disabled.** Unlocking a frame that is not drawn is not a coherent request, and the refusal line names `/<slash> enable`, which is the step the player actually needs.
...
- **A full `lock` / `unlock` pair at top level** — Absorb Tracker, Aura Master, Panel Master, Party Frame Enhanced — is the canonical shape. Nothing changes for them beyond confirming the write goes through the one seam and that the slash-surface subsection above refuses both while disabled.
