...
- **MUST** ship a **test mode** — the preview as a mode of its own, turned on and left on until turned off, independent of the lock, session-only, and ended when combat starts — and expose it as the **`Test mode`** checkbox in General → Master controls (options-ui-§15). Unlocking MAY also show the preview, and a one-shot test verb MAY stay beside the mode, but neither replaces it.
