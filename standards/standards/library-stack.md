...
| `LibKa0s-DebugLog-1.0` | `DebugLog.lua` | the on-screen debug console and its copy window, both line formatters, the 1500-line buffer, and the enable seam (debug-logging) |
...
| `LibKa0s-Launcher-1.0` | `Launcher.lua` | the one minimap button and LibDataBroker plugin every addon carries, registered as ONE object rather than two so the icon, the label and both clicks can never disagree between the minimap and a broker display. Owns the rung dispatch (launcher-§2) and the `hide` inversion that keeps LibDBIcon's own stored key the single record of whether the button shows. **Launcher minor 3 (`LibKa0s v1.57.0`)** also owns the **status tooltip** launcher-§1 requires: it always sets the object's `OnTooltipShow` and draws the label, the *Enabled* / *Locked* / *Test mode* lines and the click hints from the descriptor's optional `isLocked`, `isTestMode`, `leftClickLabel` and `version`, appending the host's own `onTooltipShow` lines between the status block and the hints. It had shipped for some time without a row here, which is the omission that made this table's own counts wrong |
