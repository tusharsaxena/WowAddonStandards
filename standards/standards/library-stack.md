...
**The modules.** `LibKa0s` ships **twelve LibStub majors across eighteen files** — `Core`, `Env`, `Item`, `Pool`, `Media`, `Widgets`, `DebugLog`, `Slash`, `Options`, `Perf`, `Launcher`, `Lifecycle` — loaded by one aggregate `LibKa0s.xml`, plus one non-code payload (`media/`, library-stack-§8). The file count is not the major count and never was: `Options` spans five files, `Widgets` and `Perf` two each. **Both figures are recounted, not incremented, and this table has now fallen behind twice.** They read *ten across fourteen* while `Launcher` was already shipping and were then incremented by one for `Lifecycle`, which carried the omission forward and made the recount look like arithmetic. The correction that was meant to end that landed **eight hours after `OptionsTabs.lua` was committed to the library** and still counted seventeen: the second drift was not a stale reading but a recount performed against the prose instead of against the tree. Recount against `LibKa0s/tests/majors.lua`, the library's manifest of every major and the files that make it up: `tests/run.lua` and `tools/gen-api-members.lua` both read it, so a file added to `LibKa0s.xml` and not to it fails the library's own versioning suite instead of going quiet here:
...
**Inter-module dependencies (MUST).** **Eleven of the twelve majors** need `LibKa0s-Core-1.0` — for secret-safe stringification, the window skin, or both — i.e. every module except `Core` itself, and **seven of those eleven** gate on it without calling a single member, reading nothing but its `MINOR` to clear the floor — `Lifecycle` among them, which needs nothing from Core and floors on it for this reason alone — so that a host holding a partial payload gets *every* module absent rather than a mixed set. That is the majority behavior and not an edge case: only `Widgets`, `DebugLog`, `Slash` and `Perf` ever reach a Core member. There are exactly three second edges: `LibKa0s-DebugLog-1.0` also floors on `LibKa0s-Widgets-1.0`, since its copy window is that module's; `LibKa0s-Perf-1.0` also floors on `LibKa0s-Lifecycle-1.0`, because a suspended arm is a hold on that latch and a harness that could not take one would have to reach past it into the host; and `LibKa0s-Options-1.0` floors on `LibKa0s-Pool-1.0`, whose tab-strip buttons and page content panel it takes from the pool rather than rebuilding them per page. The third edge differs from the other two in **where it is borne**: it is declared in the attach files (`OptionsWidgets.lua:33`, `OptionsTabs.lua:38`) rather than in the major's shell, so a reader checking the shell alone does not find it. Every dependency is declared and enforced in exactly one direction.
