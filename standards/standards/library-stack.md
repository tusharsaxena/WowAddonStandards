...
**The modules.** `LibKa0s` ships **twelve LibStub majors across seventeen files** — `Core`, `Env`, `Item`, `Pool`, `Media`, `Widgets`, `DebugLog`, `Slash`, `Options`, `Perf`, `Launcher`, `Lifecycle` — loaded by one aggregate `LibKa0s.xml`, plus one non-code payload (`media/`, library-stack-§8). The file count is not the major count and never was: `Options` spans four files and `Perf` two. **Both figures are recounted, not incremented.** They read *ten across fourteen* while `Launcher` was already shipping and were then incremented by one for `Lifecycle`, which carried the omission forward and made the recount look like arithmetic — the failure mode a count in prose invites, and the reason the sentence below names the one list that cannot be a release behind. Count it against `LibKa0s/LibKa0s.xml`'s `Script` entries, which is the one list that cannot be a release behind:
...
| `LibKa0s-Launcher-1.0` | `Launcher.lua` | the one minimap button and LibDataBroker plugin every addon carries, registered as ONE object rather than two so the icon, the label and both clicks can never disagree between the minimap and a broker display. Owns the rung dispatch (launcher-§2) and the `hide` inversion that keeps LibDBIcon's own stored key the single record of whether the button shows. It had shipped for some time without a row here, which is the omission that made this table's own counts wrong |
...
**Inter-module dependencies (MUST).** **Eleven of the twelve majors** need `LibKa0s-Core-1.0` — for secret-safe stringification, the window skin, or both — i.e. every module except `Core` itself, and three of the ten gate on it without calling a single member — `Lifecycle` among them, which needs nothing from Core and floors on it for this reason alone — so that a host holding a partial payload gets *every* module absent rather than a mixed set. There are exactly two second edges: `LibKa0s-DebugLog-1.0` also floors on `LibKa0s-Widgets-1.0`, since its copy window is that module's; and `LibKa0s-Perf-1.0` also floors on `LibKa0s-Lifecycle-1.0`, because a suspended arm is a hold on that latch and a harness that could not take one would have to reach past it into the host. Every dependency is declared and enforced in exactly one direction.
