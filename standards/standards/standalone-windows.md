...
- **SHOULD** centralize the window's look in a single `SKIN` table + one `ApplySkin(frame)` re-skin seam, built from **stock Blizzard textures** (no shipped art), so a future settings-driven re-skin has one touch point. The debug console (debug-logging) reuses this same seam.
