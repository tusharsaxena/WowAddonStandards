...
- Provide **two pure formatters** (frame-free, unit-tested) so the colored string can't drift from the plain one:
...
- Slash (slash-commands): `/<slash> debug` **toggles the window only** (state untouched); `/<slash> debug on` and `/<slash> debug off` set the flag. Each state change **MUST** print a `NS.PREFIX`-tagged chat ack whose **state word is color-coded** — **`ON` green (`40ff40`)**, **`OFF` red (`ff4040`)** — e.g. `[XY] debug logging |cff40ff40ON|r` / `[XY] debug logging |cffff4040OFF|r`. The colors mirror the title-bar toggle's green/red (below) so the flag reads identically in chat and on the console header.
