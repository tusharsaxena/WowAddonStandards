...
- The runner, and any profile-preparation step called with it at initialization and from AceDB's profile callbacks, is the **load pass**: it writes stored data directly, before any reader has seen it, and is reachable from nothing else. Where it seeds or repairs a **structural registry**, architecture-§5 makes it part of that registry's writer surface and requires it named in `docs/ARCHITECTURE.md` beside the writer.
