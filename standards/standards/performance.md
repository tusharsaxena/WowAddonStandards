...
- The panel is the lib's, styled by the host: the addon **MAY** pass a decoration hook for chrome the library does not draw (debug-logging-§12).
- **The panel closes with the same mark as the debug console (MUST).** They are the two windows a user opens together in the same session, so a `×` on one beside the shared `close` icon on the other is the most visible drift the collection can produce. From `PerfPanel.lua` minor 4 (LibKa0s v1.10.2) the library gets this right on its own — a host that passes **no** decoration hook gets the mark, because the panel hands `addonName or name` to `Core.MakeCloseButton`. A host that **does** pass one owns the corner and therefore owns this rule: it **MUST** build the control through the addon's single `MakeCloseButton` wrapper (standalone-windows), never with a bare two-argument call. A decoration hook that exists **only** to draw a close button **SHOULD** be deleted in favor of the library's path, which is now the shorter way to the same button (anti-patterns #65).
...
**This contract is no longer perf's alone, and the machinery it describes is the collection's stand-down seam.** slash-commands-§7 makes the **disabled** state total and builds it on exactly this — the same unregistration, the same cancellation, the same show-decision gate — because every addon already ships it here and disable had simply declined to use it. Suspend and disable are therefore **two named holds on one latch** (`LibKa0s-Lifecycle-1.0`, library-stack-§7), not two teardown paths: the harness takes the `perf` hold rather than calling the host's inert routine directly, and **releasing one hold MUST NOT stand up an addon the other is still holding down**. Everything below is unchanged in force; what changes is that *resume* means **release this hold and re-evaluate**, never a bare stand-up. A player can disable the addon during a suspended arm, and re-enable it there too, so the trap is reachable rather than theoretical.
