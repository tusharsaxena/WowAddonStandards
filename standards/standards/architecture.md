...
- **MUST NOT** write any other persistent state outside the helper without a `Documented deviations` row. State that is neither a row nor registry membership and bookkeeping — geometry only a drag writes, on a registry member or not, a remembered view — keeps exactly the obligation it had before the registry clause existed (debug-logging-§10 already treats it as non-schema).
