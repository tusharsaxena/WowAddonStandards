...
- Implementation: AceEvent-3.0's `:SendMessage`/`:RegisterMessage` (already in the addon). **The bus is AceEvent, and no library replaces it.** `LibKa0s-Bus-1.0` (from `LibKa0s v1.55.0`, library-stack-§7) does not: it is the sanctioned **stand-down record** for **tracked** receivers — `bus:NewTarget()` answers a fresh AceEvent target per call, so it satisfies the receiver rule above, and the library records that target's registrations so `StandDown` and `StandUp`, called from inside the host's latch callbacks, take them down and replay them (slash-commands-§7) — plus the strict `Catalog` above. A host's **own untracked factory**, the `NS.NewBusTarget` printed above, **remains permitted**. Whether a settings panel's subscription to its own refresh message may survive a stand-down is recorded, not ruled (open-evolutions), and this section takes no side in it. This is **permissive, not a new MUST**: no addon is required to adopt the major by this release, and a host whose stand-down record is its own is not out of compliance with this section.
