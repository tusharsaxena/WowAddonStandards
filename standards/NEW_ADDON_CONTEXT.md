...
20a. `README.md` follows the **canonical section order** (documentation-§1): title → badges (`[wow]`/version/`[tests]`/license/standard) → logo → description → Screenshots → Usage (Slash commands + Settings panel tables) → How it works → FAQ → Troubleshooting → **Issues and feature requests** (→ GitHub issues) → Testing → Version History. TOC follows the fixed field order + `#`-section file listing (toc-file-§1/toc-file-§5).
