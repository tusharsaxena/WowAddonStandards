# New Ka0s Addon — Context Pack (v2.69.0, 2026-09-26)
...
<Addon>/
  ...
  core/                  -- dependency-correct order, NOT a fixed prefix (layout-§1); the
    ...
    Compat.lua           -- deprecated-API shims; conventional (reached at call time); only when the addon owns one (compat)
  ...
  modules/
    <Feature>.lua
    Diagnostics.lua      -- the sections of `/<slash> diagnostics`; the library writes the rest (debug-logging-§14)
  ...
  docs/                  -- ARCHITECTURE.md, testing.md, smoke-tests.md, planning; NO agent-context.md (documentation-§3); no TODO.md once released (documentation-§4)
    ...
    perf-analysis/       -- required: the standing IN-GAME capture store (performance-§8)
      README.md          --   the store doc + the capture index; the one file rewritten
...
## Version: 0.1.0
## IconTexture: Interface\AddOns\<Folder>\media\logos\<addon>.logo.128.tga
...
# Modules
modules\<Feature>.lua
modules\Diagnostics.lua                  -- conventional: the descriptor reads its sections at RUN time
...
function addon:OnInitialize()
  ...
  NS:RunMigrations()     -- Database.lua; before anything reads NS.db.profile (savedvariables-§1)
  NS.CreateOptionsPanel() -- EAGER settings-category registration (options-ui-§1) — always visible in
                          ...
                          -- Called in combat, the library parks it and replays it once at
                          -- PLAYER_REGEN_ENABLED (options-ui-§5); add no park of your own here.
end

function addon:OnEnable()
  -- Subscribe through Core's pcalled helper, never a bare RegisterEvent (events-frames-taint-§1):
  -- a name this build lacks lands in NS.rejectedEvents and every other event still binds.
  -- NS.SafeRegisterEvent(self, "PLAYER_ENTERING_WORLD", "OnEnter", NS.rejectedEvents)
end
```

### `Compat.lua`

Ship this file only if the addon makes a deprecated or version-variant client call outside `LibKa0s`'s majors, including a `LibKa0s-Compat-1.0` member it consumes (compat's applicability condition). An addon with none omits it, drops its TOC line, and records `compat-layer` as *Not applicable* in the Documentation map, citing the condition.
...
NS.Util = NS.Util or {}
-- The HOST owns the rejected-event list: the library keeps no state and prints nothing, so the
-- addon surfaces this list in its [Init] summary or a debug verb (events-frames-taint-§1).
...
if not lib then
  ...
  -- owes the caller the SAME NAMES the live seam publishes — the three chrome names and the three
  -- registration helpers — or a module that reads them loads against nil.
  NS.SKIN            = {}
  NS.ApplySkin       = function() end
  NS.MakeCloseButton = function() return nil end

  -- The SafeRegister* family as ONE-RUNG bodies (events-frames-taint-§1): a plain pcall with no
  -- C_EventUtils front gate, the failed name appended to `rejected` once. Dropping them would leave
  -- a degraded load with bare registrations, the silence the rule exists to prevent.
...
-- Every event registration in the addon goes through these (LibKa0s v1.56.0, events-frames-taint-§1).
-- They front-gate with C_EventUtils.IsEventValid when it exists and pcall otherwise, on an
-- AceEvent-embedded object and a plain frame alike.
...
An addon that adopts `LibKa0s-Schema-1.0` for this seam declares `writeThrough` in its descriptor,
the paths of the composed Master-controls rows its host verbs write (`writeThrough = { "enabled",
"locked" }`, from Schema minor 2, LibKa0s v1.56.0), and hands the **same list** to its
runtime-completing stub. On a load where the composers are hollow, a write to a listed path with no
row is stored as given, so `/<slash> enable` still lands (options-ui-§1, route (a)). The list is
path strings and nothing else: no row, label, widget, default or `validate` written by hand
(anti-pattern #73), and the value's default still comes from the AceDB defaults table.
...
NS.COMMANDS = {
  ...
  {"enable",   "Enable the addon",                           function()     NS.SetByPath("enabled", true)  end},
  {"disable",  "Disable the addon",                          function()     NS.SetByPath("enabled", false) end},
  ...
  {"debug",    "Toggle the debug console — `on`/`off` enable/disable logging",
    function(rest)
      -- DebugVerb tests `diagnostics` FIRST, then `on` / `off`, and answers false for any other
      -- word (debug-logging-§5, §14). Bare `debug` and an unknown word toggle the WINDOW, the flag
      -- untouched. A retired `diag` or `dump` is one of those unknown words, with no hint.
...
if not SlashLib then
  ...
  -- library's rendering into the stub — no FormatRow, no parser, no `key = value` shape; a help row
  -- prints `cmd  desc` plainly. Minimal OnSlash dispatch (split the message, look the verb up in
  -- NS.COMMANDS, call its handler) is the host's own routing and is sanctioned (slash-commands-§1).
  --
  -- The ONE library string the stub MAY carry verbatim is DISABLED_LINE_FORMAT, so a disabled
  -- addon's refusal reads the same either way. Pin it against the live major with
  -- Kit.assertLibraryConstant (test-kit revision 26), so a one-byte drift turns the suite red.
  --
  -- A host verb that writes a COMPOSED row (enable/disable, lock, a test-mode switch) takes one of
  -- options-ui-§1's two routes on this load: write through the schema seam's `writeThrough` list
  -- (the SHOULD for enable/disable), or refuse with the library-absent line and write nothing:
  --   print(NS.L["%s is unavailable: the LibKa0s library did not load."]:format("/<slash> enable"))
  -- Either way it never raises and never acknowledges a write that did not land.
end
...
it (performance-§4). `diagnostics` is reserved too and is registered in **every** addon, with no
alias of any kind — no `diag` row, no `debug diag` word, no `aliases` entry (slash-commands-§2,
debug-logging-§14). It is on the library's live list, so it answers while the addon is disabled.
...
if not lib then
  ...
  --
  -- The composer members (MasterControls and the font, border, bar and color-pair composers) exist
  -- and answer {}: HOLLOW, never a host copy of a composed block (anti-pattern #73). The host verbs
  -- that write those rows take the two routes the Slash stub names (options-ui-§1).
  return
end
...
A page shown in combat by Blizzard's own sidebar is **locked** by the library (cover over the whole
page, tab strip included, every write refused until `PLAYER_REGEN_ENABLED`); the host adds no page-level
lock of its own (a setter that rebuilds frames still gates itself) and never closes the settings window in combat (options-ui-§2, anti-pattern #88).
...
The console window, the copy window, both formatters, the 3000-line buffer, the scrollbar, the line
...
if not lib then
  ...
  -- (debug-logging-§14); hundreds of report lines in chat would be worse than none:
  --   RunDiagnostics = function()
  --     NS.Print(NS.L["%s is unavailable: the LibKa0s library did not load."]:format("/<slash> diagnostics"))
  --     return 0
  --   end
...
- **Ship the diagnostics dump from v0.1.0** (debug-logging-§14, **MUST**, small addons included). Two
  forms and no third: the `diagnostics` row and `debug diagnostics`, both reaching
  `NS.DebugLog:RunDiagnostics()`. It runs while the addon is disabled, writes through the ungated
  append with the debug flag untouched, appends and never clears, and stays under the library's cap.
  All of that is the library's. The addon writes its **sections** in `modules/Diagnostics.lua`
  (below) and documents them in `docs/debug.md`. Needs `LibKa0s v1.60.0` or later.

### `modules/Diagnostics.lua` — the report's sections (debug-logging-§14)

The library writes everything around the sections: both markers, its half of the identity header
(client build, locale, debug flag, both combat reads, the running LibKa0s minors, the `[Init]`
summary), a `pcall` per section so a raise costs one line, the cap and the `truncated` line. The
addon writes **sections only**. A hand-rolled line buffer, `pcall` wrapper, marker or `Add` loop is
anti-pattern #90. The `out` members are documented in the library's `DebugLog` API document.

```lua
local addonName, NS = ...
-- modules/Diagnostics.lua — the addon's sections of `/<slash> diagnostics` (debug-logging-§14).
NS.Diagnostics = NS.Diagnostics or {}

-- Read state, never act: no hold, no event, no timer, no stand-up, and safe in combat. `out:add`
-- runs every argument through safeToString and formats %s-only, so pass RAW values, never a string
-- you built with `..` or string.format, and never a %d.
function NS.Diagnostics.Sections()
  local LC = NS.Lifecycle                     -- the addon's one stand-down latch (slash-commands-§7)
  return {
    {"identity", function(out)                -- the host half: what only this addon knows
      out:add("Diag", "schema stored=%s code=%s", NS.db.global.schemaVersion, NS.SCHEMA_VERSION)
      out:add("Diag", "profile=%s", NS.db:GetCurrentProfile())
      out:add("Diag", "enabled=%s stoodDown=%s", NS.GetSetting("enabled"), LC and LC:IsDown())
      out:joined("Diag", "holds", LC and LC:Holds() or {})
    end},
    {"settings", function(out)                -- `path = value (default)` for each row that differs
      out:nonDefaults(NS.Schema, NS.GetSetting, function(row) return row.default end, nil,
        { always = { "enabled" } })
    end},
    {"state", function(out)                   -- DOMAIN state: what the addon is working with
      if LC and LC:IsDown() then out:add("Diag", "stood down: runtime state released") return end
      -- one line per fact a maintainer would otherwise ask for; an unbounded list takes the
      -- per-list cap: out:list("Diag", "tracked", ids)
    end},
    {"rejected", function(out)                -- the events SafeRegisterEvent refused
      out:list("Diag", "rejected events", NS.rejectedEvents or {})
    end},
  }
end
```

Stored configuration prints the same whether the addon is enabled or stood down. A value whose
escapes are the evidence (a chat format string) goes through `out:escape`, which doubles `|`.
...
The diagnostics dump's contract cases are not written here either: the kit's shared
`test_diagnostics_contract.lua` runs them against **this** addon's dispatcher — both forms, while
disabled, append, the flag untouched, the markers, `debug diag` not running the report — declared by
...
Root ships exactly three docs plus `LICENSE`, and never a fourth: a **full** `README.md`, a **stub** `CLAUDE.md`, and `DEPENDENCIES.md` (documentation, documentation-§7). Everything else lives under `docs/`. The stub is short and **MUST** carry a `## Standards compliance (read first)` section (documentation-§6). **This pack is NOT copied into the addon** — see the banner at the top of this file. The `docs/` trio is `ARCHITECTURE.md`, `testing.md`, `smoke-tests.md`; five topic-detail docs are required alongside it — `test-cases.md`, `performance.md`, `perf-analysis/README.md`, `automated-tests/README.md` and `automated-tests/RESULTS.md` (documentation-§3).
...
**One entry: Pillow**, and only for **regenerating** the launcher logo —
`media/logos/<addon>.logo.128.tga` is committed, so nothing is generated at package time and the
recipe in layout-§4 is run by hand when the source art changes. Everything else ships from the
committed tree, and no other image or font tooling is needed. **None of this group is required to
...
8. Options UI: **consume `LibKa0s-Options-1.0`** — one instance from a descriptor in `settings/OptionsSetup.lua`, and `NS.Helpers` **is** that instance, decorated in place, never a copy-across (options-ui-§1). Never hand-roll the shell, the widget makers, the flow engine, the header or the scrollbar patch (anti-pattern #47), and never copy a `LAYOUT` constant into the addon. Behind it: Blizzard `Settings.RegisterCanvasLayoutCategory` + raw AceGUI body, **category registered eagerly at load** (always visible), **body built lazily** on first `OnShow`, and the header's **Defaults button built lazily too**, in that same first `OnShow`, never at registration time (options-ui-§5) — AceGUI is shared and UI skins hook `RegisterAsWidget`, so a widget created during load keeps Blizzard's stock red `UI-Panel-Button-Up` art forever while later-created ones come out skinned, making the look a race against addon load order. Never AceConfigDialog for content; never defer registration to first `/config`. Called under `InCombatLockdown()`, `CreateOptionsPanel` may be parked by the library and replayed exactly once at `PLAYER_REGEN_ENABLED` (Options minor 24); the host calls it once at login and adds no park of its own (options-ui-§5). This one setup file's fallback is **load-completing, not member-answering** — the load-completing exception options-ui-§1 names, because page files call into it inside schema-row literals at file load. Its composer members answer `{}` (hollow, never a host copy: anti-pattern #73), and a host verb that writes a composed row on that load either writes through the Schema seam's declared `writeThrough` list or refuses with the library-absent line, never raising and never acknowledging a write that did not land; `enable`/`disable` **SHOULD** write through (options-ui-§1).
8b. Options UI — the panel's **content**, which is the half a player compares between two addons, and all of it MUST (options-ui-§13/§14/§15/§16/§17/§18):
    - **Every settings page draws a tab strip**, one tab per `group`, **including a page with exactly one section** — the one tab that cannot be clicked is that page's section label. A strip the library draws (`O.TabStrip`, `O.RenderTabbedSchema`) has its wrap invariance pinned by the library's suite, and the addon **MUST NOT** duplicate that case (options-ui-§13, testing-§8). The untabbed heading form is for a page the host does not render through the flow engine, which today means **two** pages and both of them — the AceConfig-drawn **Profiles** sub-page and the **landing page**, whose `buildMain` body (logo, tagline, *Slash Commands* heading, one Label per `COMMANDS` row) declares no `group` and is mandated in exactly that shape by options-ui-§5; a renderer that falls back to it below some tab count is itself the defect, because it makes a page's chrome a function of how many sections it happens to hold. **Every row carries a `group`** — a row without one belongs to no tab and is an authoring defect the library reports. A wrapped strip's **geometry must not change with the selection**: the row pitch and the reserved band are one measurement taken from a state no click can change, never read off whichever tab was drawn first, because a selected tab's art is a **different atlas** from an unselected tab's. One secondary strip **MAY** divide a single primary tab's content, drawn inside the scroll rather than as a second pinned band; never a third level. A page that edits one instance out of many **MAY** instead fold its sub-pages under a pinned **nav rail** (`O.NavRail`), the first level, with the primary strip beside it the second: rail plus strip is two levels, so such a page draws no secondary strip, and the rail's selection and each entry's tab are session state (options-ui-§13).
    - **Above the strip — and the rail, where there is one — sits at most ONE chrome block** (options-ui-§14): the page's picker for the instance it edits, plus every control that applies to all of its tabs — create, enable, unlock, copy, reset, delete. A page-wide control drawn under one tab reads as belonging to that tab and vanishes when the player clicks another. The band is already delimited by its divider and the content panel's top edge, so the block is **never boxed a second time**. The rail is not a picker.
    - **The General page's first tab is named exactly `Master controls`**, with the canonical rows in canonical order: `Enable <Addon>` | `General visibility` / `Master scale` | `Master alpha` / `Lock frame` | `Debug console` / `Minimap button` | `Test mode` / `Reset position` | `Reset all settings`. `Minimap button` is **unconditional** — every addon ships a launcher — and takes the **first** column of its line precisely because only some addons have a `Test mode` to pair beside it; it stores at LibDBIcon's own `minimap.hide`, never at a second key, and its get/set invert because the label says *shown* while the boolean says *hidden*; its schema path, which is its CLI name, says *shown* too: `global.minimap.shown` (launcher-§3). Include only the rows the addon has the state for: `Test mode` — a session-only checkbox the composer emits from `testModePath` — in every addon with a positionable display, switching its test mode (placeholder content, on until turned off, ended by combat), never as a button — omitted when unlocking already shows the placeholders, where Lock frame is the switch and the addon ships no `test` verb (a one-shot value hold, if kept, lives at `/<slash> debug hold <value> [secs]`, options-ui-§15, preview-mode); a **frameless** addon omits exactly the four frame-only rows and **never invents a movable frame** to fill the tab out, and an addon whose frames are per-instance keeps the per-instance scale/alpha/lock on the instance's own page. `General visibility` is a four-value dropdown (`always` / `inCombat` / `outOfCombat` / `never`), never a *show only in combat* boolean — and replacing such a boolean is a **stored-value type change**, so it carries a migration step and a raised `NS.SCHEMA_VERSION` (never a bumped default) (`true` → `inCombat`, `false` → `always`) in the same change, never a bare type swap. `Reset all settings` is options-ui-§12's global reset, not a second one; a frameless addon draws it alone.
    - **Font, border and bar groups have fixed membership and a fixed row order**, contiguous, with extras appended **after** the block rather than between its rows: font face | size / color | use class color / flags | shadow; (show border) / style | thickness px / color | use class color; texture | opacity / color | use class color. They are **composed from one declaration**, not typed out. A group over a **background** is not a bar group and gets no texture picker. A tab mixing two of them gives each a **subsection heading** (`subgroup`), drawn with the same `Heading` widget everything else uses — never a hand-rolled colored `Label`.
    - **Every color control carries a class-color companion** in the two-column position immediately to its right — a `Use class color` checkbox, or a color-mode dropdown whose values include `class`, which is the richer form and is never downgraded to a checkbox. Default **off**. One resolver for all of them: the stored swatch's **alpha survives** the mode, an **unresolvable class falls through to the stored swatch** (never gray, never white), and the swatch is read under both modes so it is **never disabled** — `disabledIf` on a color row is forbidden. The class is **the tracked unit's** where the surface describes a unit and **the player's** everywhere else; the stored path does not decide this, so declare it on the row (`classColorSource = "player" | "unit"`). Palette-definition swatches — one color per statistic, per rarity, per category — are the only exemption.
...
8a. Slash: **consume `LibKa0s-Slash-1.0`** — the dispatcher, help renderer, formatters, list builder and type-aware parser (slash-commands-§1). The addon keeps its ordered `NS.COMMANDS` table of **positional triples** `{name, desc, fn}` and passes it in; the library never owns it and never registers a chat command. Reserved verbs — `help get set list reset resetall config version debug perf diagnostics` — mean the same thing in every Ka0s addon, and `diagnostics` is registered in every addon with no alias (debug-logging-§14); `reset` takes a **path**, not a page. `enable`/`disable` are reserved at the **top level**; a feature noun's sub-tree (`/<slash> <noun> enable <id>`) **MAY** reuse them for that noun's items (slash-commands-§2). The library-absent Slash stub carries no library string but `DISABLED_LINE_FORMAT`, pinned with `Kit.assertLibraryConstant`, never copies `FormatRow`, and a verb it refuses prints the one library-absent line `%s is unavailable: the LibKa0s library did not load.` through the locale (slash-commands-§1).
...
11. All deprecated-API calls live in `Compat.lua`, which exists only when the addon has one (compat's applicability condition). Modules call `NS.Compat.X`. No `WOW_PROJECT_ID` flavor branching (Retail only).
11a. Events (events-frames-taint-§1): AceEvent-3.0, and every registration goes through `LibKa0s-Core-1.0`'s `SafeRegisterEvent` / `SafeRegisterUnitEvent` / `SafeRegisterEvents`, published from `core/CoreSetup.lua`, never a bare `RegisterEvent` and never a private copy of the loop. The **host** owns the rejected list (`NS.rejectedEvents`) and surfaces it in the `[Init]` summary or a debug verb. The library-absent Core stub carries the three as one-rung `pcall` bodies. The only private frames are the unit-filter frame and, for an addon that embeds **no** AceEvent-3.0, one lazily created boundary watcher for `PLAYER_REGEN_DISABLED`/`_ENABLED`.
...
- **A diagnostics dump under any other name, or built by hand** (anti-pattern #90) — a `diag`, `dump` or `dx` verb, word or alias that runs the report, a report written through the gated sink, a `Clear()` before it, a report refused while disabled, or a host-written line buffer, `pcall` wrapper and marker set in place of `RunDiagnostics`.
...
- Deferring settings-**category** registration until first `/config`/panel-open (register eagerly at load), or wrapping the `CreateOptionsPanel` call in a host-side combat park (the library parks and replays it once, options-ui-§5).
...
- [ ] Any structural registry (a collection the player adds to and removes from) has **one** registry writer that the panel and the slash verbs both call, and `docs/ARCHITECTURE.md` → Settings Schema names its storage keys, that writer and the load pass; no panel or slash file appends to or removes from the stored collection itself (architecture-§5, anti-pattern #78).
- [ ] Any state written outside the write seam that no control sets and no row addresses — geometry only a drag or a resize determines, a remembered view, learned or recorded data, a vendored library's own writes — is named in `docs/ARCHITECTURE.md` → Settings Schema with its storage key, its one owner module and every function that writes it (a listed writer outside the owner module is compliant), and carries no register row; anything else outside the seam has a row or a register row (architecture-§5).
...
- [ ] **`media/logos/<addon>.logo.128.tga` exists, is 128×128 and is uncompressed 32-bit**, `## IconTexture` points at it and the LDB object carries the same path — not a Blizzard icon, not a file id, not an RLE TGA (layout-§4, toc-file-§1, anti-pattern #82).
- [ ] **`/<slash> enable` and `/<slash> disable` exist** as aliases writing the *Enable* row's stored path through the single write seam, holding no state of their own (slash-commands-§2).
- [ ] **Disabled means the addon is not running** (slash-commands-§7) — frames hidden, every event, message and bucket registration **actually unregistered** (an early-returning handler does **not** satisfy this: the addon stopped reacting, not watching, and still pays the dispatch), every timer, ticker and `OnUpdate` canceled, no SavedVariables write from any game event, and all of it through the **same latch the perf harness's suspend takes a hold on** — never a second teardown path. The chat command, `NS.COMMANDS`, the dispatcher, the settings-category registration and AceDB's profile callbacks are **setup** and stay up in either state. **Every verb keeps answering while disabled** (slash-commands-§2): every reserved verb — `help`, `config`, `version`, `enable`, `disable`, `debug`, `diagnostics`, `perf` — the bare `/<slash>`, which opens the panel, and the whole schema CLI (`get`, `set`, `list`, `reset`, `resetall`). Only a **feature** verb SHOULD refuse, printing **one** tagged line — `<tag> <Ka0s Name> is disabled — enable it with |cFFFFFF00/<slash> enable|r` — and doing nothing else. The launcher's left-click still opens the panel; its right-click menu keeps *Enabled* live and grays every other entry, and **MUST NOT** write except through *Enabled*. Vendor LibKa0s **v1.42.0** or later — `LibKa0s-Lifecycle-1.0` ships from v1.40.0, and v1.42.0's Slash minor 14 is the first dispatcher that answers the restored surface exactly. Ship `tests/test_disabled.lua` (slash-commands-§7, *The conformance test every addon ships*).
- [ ] **Font, border and bar groups are composed, contiguous and in canonical order**, with a border thickness slider and a bar opacity slider present rather than left as literals in the render path, extras appended after the block, and a `subgroup` heading per group on any tab that mixes them (options-ui-§16, options-ui-§7).
...
- [ ] `README.md` is player-facing and plain-language (no contributor material, no `## Testing` section) and follows the canonical section order (documentation-§1) with **no logo image** (anti-pattern #79), including **Usage** (prose, five paragraphs or fewer, no tables, closing on one line pointing at Settings → AddOns and the slash command list), **Reporting a bug** (documentation-§1 item 9, verbatim apart from the real slash), **Issues and feature requests** (→ GitHub issues), and **Version History**. Every edit to it has been through a de-AI writing pass (`/humanize` or equivalent) — the README only; `docs/` and `CLAUDE.md` are contributor surfaces and exempt.
...
| Combat-lockdown cascade | A layered `InCombatLockdown()` guard on secure writes (settings-setter → secure-frame-show), each deferring on `PLAYER_REGEN_ENABLED`. **Config-open is the exception: it refuses with a gray notice, never defers**, and a page shown in combat is locked by the library, never closed (options-ui-§2). |
| Eager settings registration + lazy body | Register the Blizzard **category** at load (bootstrap on `ADDON_LOADED(Blizzard_Settings)`/`PLAYER_LOGIN`, or in `OnInitialize`); build the panel body only in the first `OnShow`. A call made in combat is parked by the library and replayed once at `PLAYER_REGEN_ENABLED`, never by the host. |
...
| Diagnostics dump | Two forms and no third — a `diagnostics` verb and a `debug diagnostics` word, the word tested first — both reaching the console library's one report runner, which writes through the ungated append, never clears, runs while the addon is disabled and stays under a cap below the buffer. The addon hands it a brand for the markers and a run-time function returning named sections, and writes those sections and nothing else. |
| Debug coverage that reads back | Gated one-line traces at the main functional flows *including the not-recorded decisions*, a repeating pass collapsed to a single summary line carrying its counts and id lists (with the list-building itself behind the gate), and settings changes logged once at the write seam rather than re-echoed by every reactor, with a bulk copy or reset through the seam collapsed to one line carrying its row count. |
