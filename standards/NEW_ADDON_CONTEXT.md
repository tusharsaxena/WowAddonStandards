# New Ka0s Addon — Context Pack (v2.54.0, 2026-09-16)
...
- [ ] **Disabling stands the features down, not the command surface** — the chat command, `NS.COMMANDS`, the dispatcher and the settings registration come up in either state and stay up, and `help`, `config`, `version`, `enable`, `disable`, `debug`, `perf` and the schema CLI (`get`, `set`, `list`, `reset`, `resetall`) all keep answering while the addon is off. A **feature** verb **SHOULD** refuse on one tagged line naming `/<slash> enable` and do nothing else (slash-commands-§2).
