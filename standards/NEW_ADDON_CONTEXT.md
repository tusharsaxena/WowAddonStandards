# New Ka0s Addon — Context Pack (v2.60.0, 2026-09-19)
...
page, tab strip included, every write refused until `PLAYER_REGEN_ENABLED`); the host adds no guard
of its own and never closes the settings window in combat (options-ui-§2, anti-pattern #88).
...
- [ ] Combat-lockdown: secure writes defer on `PLAYER_REGEN_ENABLED`; options-panel open **refuses** under lockdown (gray notice, no defer — options-ui-§2); no addon code closes or hides `SettingsPanel` in combat, and no host-side combat guard sits beside the library's page lock (options-ui-§2/§13, anti-pattern #88).
